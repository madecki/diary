 ...
 *   - Sets ownerId on all emotions, triggers where ownerId = 'UNASSIGNED'
 *   (Projects/tags live in settings-service; assign those there if needed.)
 *
...
async function main() {
  ...
  const [entries, emotions, triggers] = await Promise.all([
