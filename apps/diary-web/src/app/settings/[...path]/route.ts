/**
 * E2E-only proxy: `/settings/:path*` → settings-api (`E2E_SETTINGS_API_TARGET`).
 * Mirrors `app/diary/[...path]/route.ts` so browser `fetch("/settings/...")` works in Playwright.
 */
import { type NextRequest, NextResponse } from "next/server";

const E2E_SETTINGS_API_TARGET = process.env.E2E_SETTINGS_API_TARGET;
...
async function proxy(req: NextRequest, params: Promise<{ path: string[] }>): Promise<NextResponse> {
  if (!E2E_SETTINGS_API_TARGET || !E2E_SERVICE_TOKEN || !E2E_USER_ID) {
    return NextResponse.json({ error: "Not found" }, { status: 404 });
  }

  const { path } = await params;
  const targetUrl = `${E2E_SETTINGS_API_TARGET}/${path.join("/")}${req.nextUrl.search}`;
