...
test.describe("API – POST /entries/short-notes", () => {
  test("creates a short note and returns the full entry", async ({ request }) => {
    ...
    expect(entry.id).toBeTruthy();
    expect(entry.type).toBe("short_note");
    expect(entry.plainText).toBe("Quick API test note.");
    expect(entry.checkInType).toBeNull();
  });

  test("creates a short note with optional title", async ({ request }) => {
    const res = await request.post(`${API_URL}/entries/short-notes`, {
      data: {
